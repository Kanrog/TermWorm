...
**A one-click, embedded Linux terminal for Klipper and Mainsail/Fluidd.**

TermWorm bridges the gap between your 3D printer's web interface and the host operating system. With a single command, it installs a web-based terminal emulator (`ttyd`) and automatically injects a native shortcut button right into your Mainsail/Fluidd sidebar. 
...
* Anyone who can access your Mainsail/Fluidd web interface on your network will have full command-line access to your printer's host OS. 

## Features
* **Zero Configuration:** Automatically detects your host IP and binds the correct ports.
* **Native Mainsail/Fluidd Integration:** Injects a clean, native UI icon into the Mainsail/Fluidd sidebar.
...
Once the script finishes, simply **refresh your Mainsail/Fluidd browser tab**. You will see a new "Linux Terminal" icon in the left-hand sidebar. 
...
3. Writes a `navi.json` file to your `~/printer_data/config/.theme/` directory to generate the Mainsail/Fluidd UI button.
